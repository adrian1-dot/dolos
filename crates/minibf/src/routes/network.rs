...
impl<'a> IntoModel<Vec<NetworkErasInner>> for ChainModelBuilder<'a> {
    ...
    fn into_model(self) -> Result<Vec<NetworkErasInner>, StatusCode> {
        ...
        // Special handling for initial era parameters: prefer authoritative
        // values from the chain state (Earliest EraSummary) when available;
        // otherwise fall back to sensible defaults from genesis or historical
        // constants for UI compatibility.
        let known_hardforks = [2, 3, 4, 5, 7, 9];

        // derive epoch/slot params from the earliest EraSummary if present,
        // otherwise fall back to genesis values
        let (derived_epoch_length, derived_slot_length, derived_safe_zone) = if let Some((_, earliest)) =
            self.eras.iter().min_by_key(|(_, e)| e.start.slot)
        {
            (
                earliest.epoch_length as i32,
                earliest.slot_length as i32,
                dolos_cardano::mutable_slots(self.genesis) as i32,
            )
        } else {
            (
                self.genesis.shelley.epoch_length.unwrap_or_default() as i32,
                self.genesis.shelley.slot_length.unwrap_or_default() as i32,
                dolos_cardano::mutable_slots(self.genesis) as i32,
            )
        };

        let mut previous = match self.genesis.shelley.network_magic {
            Some(dolos_core::MAINNET_MAGIC_U32) => {
                // keep historical end_epoch for mainnet display, but derive the
                // epoch/slot sizes from state/genesis
                let epoch_length = derived_epoch_length;
                let slot_length = derived_slot_length;
                let safe_zone = derived_safe_zone;
                let end_epoch = 208;

                NetworkErasInner {
                    start: Box::new(NetworkErasInnerStart { time: 0, slot: 0, epoch: 0 }),
                    end: Box::new(NetworkErasInnerEnd {
                        time: end_epoch * epoch_length * slot_length,
                        slot: end_epoch * epoch_length,
                        epoch: end_epoch,
                    }),
                    parameters: Box::new(NetworkErasInnerParameters { epoch_length, slot_length, safe_zone }),
                }
            }
            Some(dolos_core::PREPROD_MAGIC_U32) => {
                // keep historical end_epoch for preprod but derive the params
                let epoch_length = derived_epoch_length;
                let slot_length = derived_slot_length;
                let safe_zone = derived_safe_zone;
                let end_epoch = 4;
            ...
            Some(dolos_core::PREVIEW_MAGIC_U32) => {
                // For Preview we still add the skipped eras but derive epoch/slot
                // sizes from state/genesis rather than literals.
                let epoch_length = derived_epoch_length;
                let slot_length = derived_slot_length;
                let safe_zone = derived_safe_zone;
                let end_epoch = 0;
                ...
                // In the case of preview, we add the skipped eras using genesis
                // epoch/slot sizes to ensure these entries match the genesis.
                let other = NetworkErasInner {
                    start: Box::new(NetworkErasInnerStart { time: 0, slot: 0, epoch: 0 }),
                    end: Box::new(NetworkErasInnerEnd {
                        time: end_epoch * (self.genesis.shelley.epoch_length.unwrap() as i32) * (self.genesis.shelley.slot_length.unwrap() as i32),
                        slot: end_epoch * (self.genesis.shelley.epoch_length.unwrap() as i32),
                        epoch: end_epoch,
                    }),
            ...
            _ => {
                // Unknown / private networks: use the derived values (from state
                // if present, else from genesis).
                let epoch_length = derived_epoch_length;
                let slot_length = derived_slot_length;
                let safe_zone = derived_safe_zone;
                ...
                NetworkErasInner {
                    start: Box::new(NetworkErasInnerStart { time: 0, slot: 0, epoch: 0 }),
                    end: Box::new(NetworkErasInnerEnd { time: 0, slot: 0, epoch: 0 }),
                    parameters: Box::new(NetworkErasInnerParameters { epoch_length, slot_length, safe_zone }),
                }
            }
        };

        // Ensure we return an entry for every known hardfork in the
        // canonical order. This keeps the `/network/eras` response
        // compatible with clients that expect the full era-history
        // (e.g. GeniusYield/Blockfrost provider expects exactly 7 entries).
        let eras: Vec<_> = known_hardforks
            .iter()
            .map(|protocol| {
                // Prefer an authoritative era summary from state when present.
                // Otherwise synthesize a compatible placeholder so clients
                // (andamio / Blockfrost client) receive the full array.
                match self.eras.iter().find(|(p, _)| p == protocol) {
                    Some((p, era)) => (*p, era.clone()),
                    None => {
                        let placeholder = EraSummary {
                            start: dolos_cardano::model::EraBoundary { epoch: 0, slot: 0, timestamp: system_start },
                            end: Some(dolos_cardano::model::EraBoundary { epoch: 0, slot: 0, timestamp: system_start }),
                            epoch_length: derived_epoch_length as u64,
                            slot_length: derived_slot_length as u64,
                            protocol: *protocol,
                        };
...
fn compute_eras_sync<D: Domain>(domain: Facade<D>) -> Result<Vec<NetworkErasInner>, StatusCode> {
    ...
    let builder = ChainModelBuilder { eras, genesis: &genesis, tip };
    ...
    D: Domain + Clone + Send + Sync + 'static,
{
    ...
    let fetcher = move || compute_eras_sync(domain_clone);
